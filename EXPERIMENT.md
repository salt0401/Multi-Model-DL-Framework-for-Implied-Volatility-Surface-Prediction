...
> **Status (2026-02-20):** Model 1 (SSVI+NN) has been retrained and validated on the latest codebase. Models 2-5 (HyperIV, DGM, Adjustment, DDPM) require retraining — their previous results were based on an older Model 1 checkpoint and have been removed. See "Retraining Plan" at the bottom.
...
| Input features | 12 dims (6 base + 6 enhancement, vixtwn_change removed) |
| Dropout | 0.2 |
...
> **Results: Pending retraining.** This model depends on Model 1's forward-pass outputs (`tv_pred`). Previous results used an older Model 1 checkpoint and included the since-removed `vixtwn_change` feature (13→12 input dims).
...
Models 2-5 need retraining in the following order:

1. **Model 4 (HyperIV)** — Independent of Model 1, can train in parallel
2. **Model 2 (DGM)** — Independent of Model 1, can train in parallel
3. **Model 3 (Adjustment)** — Depends on Model 1 outputs (`tv_pred`), must train after Model 1
